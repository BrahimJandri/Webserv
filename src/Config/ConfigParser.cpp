...
ConfigValue ConfigParser::parseDirectiveValue(const std::string& key, const std::string& value) {
       if(key == "error_page") { // Handle error_page directive (format: "404 500 /error.html")
            std::vector<std::string> parts = splitWhitespace(value); // Split value into parts by whitespace
            if(parts.size() >= 2) { // Need at least error code(s) and page path
                ...
                for(size_t i = 0; i < parts.size() - 1; ++i) { // Iterate through all parts except the last (page path)
                    if(i > 0) codes += " "; // Add space separator between codes
        ...
        } else if(key == "allowed_methods") { // Handle allowed_methods directive (format: "GET POST PUT")
            return ConfigValue(splitWhitespace(value)); // Return list of methods split by whitespace
        } else if(key == "port") { // Handle port directive (format: "8080")
            if(isNumber(value)) { // Check if value is a valid number
        ...
        } else if(key == "limit_client_body_size") { // Handle client body size limit (format: "10M" or "1024")
            return ConfigValue(parseSize(value)); // Parse size string (handles MB suffix) and return as int
        } else if(key == "autoindex" || key == "upload_enable") { // Handle boolean directives
...
LocationConfig ConfigParser::parseLocationBlock(size_t startIndex) {
    ...
    if(locationPos == std::string::npos) { // If "location" keyword is not found
    ...
    for(int i = static_cast<int>(startIndex); i <= endIndex; ++i) {
        ...
        if(line.find("location") != std::string::npos || // Skip lines containing "location"
           ...
           line.find("}") != std::string::npos) { // Skip lines containing closing brace
            continue; // Move to next iteration
        }
        
        // Parse directive
        std::pair<std::string, std::string> directive = parseDirectiveLine(line); // Parse the line into key-value pair
        if(!directive.first.empty()) { // If a valid directive was found
